...
enum ConnectedAdoptionDecision: Equatable {
    case adopt
...
struct Libre3DisconnectHandoffPolicy: Equatable {
    ...
    @discardableResult
    mutating func completeDisconnect(matchesTarget: Bool) -> Bool {
        // Unrelated peripherals must never release this sensor's handoff gate.
...
struct Libre3PeripheralBindingTracker: Equatable {
    ...
    mutating func recordCandidate(_ id: UUID) {
        candidateID = id
    }

    mutating func authenticatedCandidate(_ id: UUID) -> UUID? {
        // Only authentication promotes a discovered peripheral to persisted state.
        guard candidateID == id else { return nil }
        candidateID = nil
        return id
    }
...
enum Libre3ReconnectFailureCategory: String, Equatable {
    case credential
    case transport
    case other
}
...
struct Libre3GlucoseSilenceRecoveryPolicy {
    static let quietThreshold: TimeInterval = 150
    static let minimumRetryInterval: TimeInterval = 150
    ...
    static func shouldRearm(
        ...
        guard !isWarmingUp, !isExpired, !needsReplacement,
              let lastGlucoseAt,
              now.timeIntervalSince(lastGlucoseAt) >= quietThreshold else {
            return false
        }
...
struct Libre3PatchControlSequence: Equatable {
    ...
    mutating func next() -> UInt16 {
        current &+= 1
        // Sequence zero is reserved; skip it if UInt16 wraps.
        if current == 0 {
            current = 1
        }
        return current
...
final class Libre3DirectManager: ObservableObject {
    ...
    /// Receipt time for any data-plane channel. This distinguishes a dead link
    /// from a glucose-only subscription failure.
    ...
    /// Throttles glucose-only CCCD recovery without pretending traffic arrived.
    private var lastGlucoseRecoveryAttemptAt: Date?
    ...
    private var peripheralBindingTracker = Libre3PeripheralBindingTracker()
    ...
    /// The historical response channel counts as drained once no page has
    /// arrived for this long; the cap bounds a burst that never falls quiet.
    /// LibreLoop's field-tuned values, which we have no reason to differ on.
    private static let historicalDrainIdleThreshold: TimeInterval = 1.5
    private static let historicalDrainMaxWait: TimeInterval = 15
    private static let historicalDrainPollNanoseconds: UInt64 = 250_000_000
    /// Delays before each clinical attempt. A rejection immediately after the
    ...
    /// AES-CCM command nonces advance for every write attempt in one Phase-6 session.
    private var patchControlSequence = Libre3PatchControlSequence()
    ...
    /// Receipt time of the newest historical page, used to decide when the
    /// historical burst has drained and a second patchControl write is safe.
    private var lastHistoricalPageAt: Date?
    ...
    func forgetSensor() {
        ...
        // receipt timestamp must not turn the interval between sensors into a gap.
    ...
    private func adoptConnectedPeripheral(_ peripheral: CBPeripheral, reason: String) {
        ...
        guard disconnectHandoffPolicy.connectedAdoptionDecision == .adopt else {
            Libre3DiagnosticsLog.traceReconnect(
                "connected-adoption-ignored reason=\(reason) pending=disconnect-handoff"
            )
            return
        }
    ...
    private func completeLifecycleAttempt(id: UUID, error: Error?) {
        ...
        let failureCategory = error.map { Self.failureCategory(for: $0) } ?? .transport
        let errorName = error.map {
            Self.lifecycleErrorDescription(for: $0)
        } ?? "notificationStreamEnded"
        if let error {
            Libre3DiagnosticsLog.traceReconnect(
                "setup-failed stage=\(endedStage) class=\(failureCategory.rawValue) error=\(errorName)"
            )
    ...
    private func connectAuthorizeAndStream() async throws {
        ...
        if let authenticatedID = peripheralBindingTracker.authenticatedCandidate(peripheral.identifier) {
            SharedData.libre3PeripheralUUID = authenticatedID.uuidString
        }
        reconnectFailureTracker.recordAuthorizationSucceeded()
        patchControlSequence.reset()
        if reScanSuggested {
            reScanSuggested = false
            SensorAlertNotificationManager.shared.retractReconnectFailing()
    ...
    private func startSilenceWatchdog(session: SensorSession) {
        ...
        silenceWatchdogTask = Task { [weak self] in
            while !Task.isCancelled {
                ...
                if Libre3GlucoseSilenceRecoveryPolicy.shouldRearm(
                    now: now,
                    lastGlucoseAt: self.lastGlucoseAt,
                    lastAttemptAt: self.lastGlucoseRecoveryAttemptAt,
                    isWarmingUp: self.warmupRemainingMinutes != nil,
                    isExpired: self.sensorIsExpired,
                    needsReplacement: self.sensorNeedsReplacement
                ) {
                    // Keep this separate from lastGlucoseAt: recovery is not traffic.
                    self.lastGlucoseRecoveryAttemptAt = now
                    Libre3DiagnosticsLog.traceReconnect(
                        "glucose-quiet-rearm quiet=\(Int(now.timeIntervalSince(self.lastGlucoseAt ?? now)))s"
                    )
                    do {
                        try await session.refreshDataPlaneNotifications(
    ...
    private func requestBackfillIfNeeded(currentLifeCount: Int) {
        ...
        Task { [weak self] in
            do {
                ...
                // The ATT response to the historical write is NOT the end of the
                // historical burst. The sensor rejects a second patchControl
                // command while it is still answering the first, so wait for the
                // response channel to fall idle before issuing the clinical
                ...
                var clinicalAttempts = 0
                var lastClinicalError: Error?
                for backoff in Self.clinicalRetryBackoffs {
                    if backoff > 0 {
                    ...
                    clinicalAttempts += 1
                    // Every attempt gets a fresh AES-CCM nonce within this session.
                    ...
                    do {
                        ...
                        lastClinicalError = nil
                        break
                    } catch {
                        lastClinicalError = error
                        Logger.libre3.error("Libre3 BLE clinical backfill request failed (attempt \(clinicalAttempts, privacy: .public)): \(String(describing: error), privacy: .public)")
                    }
                }
                ...
                if self.session === capturedSession,
                   self.connectionState == .streaming {
                    Libre3DiagnosticsLog.traceReconnect(
                        // Page count is deliberately left to `backfill-outcome`,
                        // which reports it as a per-request delta.
                        "clinical-backfill-drain waited=\(Self.reconnectDelay(seconds: drainedAfter)) attempts=\(clinicalAttempts)"
                    )
                    if let lastClinicalError {
                        self.backfillOutcome?.clinical = .failed
                        Libre3DiagnosticsLog.traceReconnect(
                            "clinical-backfill-failed error=\(Self.compactErrorName(for: lastClinicalError))"
                        )
                    } else {
    ...
    private func ingestHistorical(_ page: HistoricalReadingPage) {
        // Counted before the anchor guard: these only answer "is the sensor
        // still replying", not "was the page usable".
    ...
    /// Wait until the historical response channel has been quiet for
    /// `historicalDrainIdleThreshold`, and return how long that took.
    ///
    /// Polled rather than event-driven: pages arrive on the shared notification
    /// stream that the session loop owns, and a once-per-session poll is far
    /// less machinery than routing a completion signal out of it. Returns early
    /// if the session is replaced, and gives up at `historicalDrainMaxWait` so a
    /// channel that never falls quiet cannot strand the clinical request.
    private func awaitHistoricalDrain(session originatingSession: SensorSession) async -> TimeInterval {
        let waitStart = Date()
        while Date().timeIntervalSince(waitStart) < Self.historicalDrainMaxWait {
            guard self.session === originatingSession else { break }
            // Before the first page the wait's own start is the reference, so a
            // burst that has not begun yet still gets one idle threshold to
            // appear. Treating "no page" as "already drained" would write the
            // clinical command into the gap between the historical ATT ack and
            // the first page — the very race this wait exists to close.
            let reference = lastHistoricalPageAt ?? waitStart
            if Date().timeIntervalSince(reference) >= Self.historicalDrainIdleThreshold { break }
            do {
                try await Task.sleep(nanoseconds: Self.historicalDrainPollNanoseconds)
            } catch {
                break
            }
        }
        return Date().timeIntervalSince(waitStart)
    }
    ...
    private static func reconnectDelay(from start: Date, to end: Date) -> String {
        reconnectDelay(seconds: end.timeIntervalSince(start))
    }

    private static func reconnectDelay(seconds: TimeInterval) -> String {
